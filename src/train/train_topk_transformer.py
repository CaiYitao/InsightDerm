    ...
    with wandb.init(config):
        ...
        for e in range(config.epochs):
            ...
            if current_score >= best_score:
                ...
                save_path = os.path.join(config.model_save_dir, f"Topk{args.topk}_{args.num_heads}heads_{args.num_layers}layers_{args.num_classes}C_{args.dataset}_weighted{args.weighted_sampling}_{args.split}_{d}.pt")
...
def main():
    ...
    config = {"grad_norm_clip": 1.02, 
            ...
            "classification":args.classification, #"mean" ,"avgpool1d" ,"LPPool1d" ,"maxpool1d" 
            "model_save_dir":"/system/user/publicwork/yitaocai/Master_Thesis/model/topk_transformer",
  
            }

    if args.weighted_sampling:
        wandb.init(project=f"CF_Transformer_{args.num_classes}C_weighted", name=f"CF_Transformer_topk{args.topk}_{args.num_heads}heads_{args.num_layers}layers_{args.classification}_seed{args.seed}_{args.split}_{args.dataset}", config=config,allow_val_change=True)
        ...
        wandb.init(project=f"CF_Transformer_{args.num_classes}C", name=f"CF_Transformer_topk{args.topk}_{args.num_heads}heads_{args.num_layers}layers_{args.classification}_seed{args.seed}_{args.split}_{args.dataset}",config=config,allow_val_change=True)
