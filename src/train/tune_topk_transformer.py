...
def main():
    ...
    'num_heads': {'values': [2,3]},
    'topk': {'values': [2,3,4,5]},
    'critical_features_from': {'values': ['original']},
    ...
    'split': {'values': ['random','determined']},
    # 'alpha': {'distribution': 'uniform','min': 0.5,'max': 1},
    'alpha': {'values': [0.849435264741892]},
    # 'grad_norm_clip': {'distribution': 'uniform','min': 0.5,'max':2},   
    'grad_norm_clip': {'values': [1.02]},
    'device': {'values': ['cuda:3']},        
    'in_size': {'values': [1000]},
    'classes': {'values': [2]},
    'out_size': {'values': [2]},
    'betas': {'values': [(0.5,0.999), (0.9, 0.999), (0.7,0.999)]},
    ...
    sweep_id = wandb.sweep(sweep_config, project=f"CF_Transformer_{args.num_heads}C_Random_search_{d}")
