...
import argparse
from utils import get_feats_for_dataset,build_testloader,get_working_df,get_feats_df,get_bag_feats,compute_roc,multi_label_roc,build_optimizer,build_scheduler,build_dataloader,plot_auc
...
def plot_confusion_matrix(label,pred,config):
  ...
  'toImageButtonOptions': {
    ...
    pio.write_image(fig, os.path.join(config.save_path,f'Topk{args.topk}_{args.num_heads}heads_{args.num_layers}_layers_{config.classes}C_confusion_matrix.pdf'),scale=6)
...
def main():
    config = {"grad_norm_clip": 1.02580285460309, 
        ...
        "AUC_save_path": "/system/user/publicwork/yitaocai/Master_Thesis/auc_test/auc_plot_topk_transformer",
        "projection":True,
        "classification":args.classification,
        "save_path": "/system/user/publicwork/yitaocai/Master_Thesis/confusionmatrix/topk_multiheadatt"
   
        }


    set_seed(args.seed)
    wandb.init(project=f"TopkCFTransformer_{args.num_classes}C_TEST", name=f"TopkCFTransformer_seed{args.seed}_{args.dataset}_{os.path.basename(args.model_path)}",config=config,allow_val_change=True)
