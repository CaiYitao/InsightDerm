...
def plot_confusion_matrix(label,pred,config):
  ...
  'toImageButtonOptions': {
    ...
    pio.write_image(fig, os.path.join(config.save_path,f'Topk{args.topk}_{args.num_heads}heads_{config.classes}C_confusion_matrix.pdf'),scale=6)
...
def main():
    config = {"grad_norm_clip": 1.02580285460309, 
        ...
        "AUC_save_path": "/system/user/publicwork/yitaocai/Master_Thesis/auc_test/auc_plot_topk_multihead",
        "projection":True,
        "classification":"mean",
    ...
    set_seed(args.seed)
    wandb.init(project=f"TopkCFMultiHeadAttention_{args.num_classes}C_TEST", name=f"TopkCFMultiHeadAttention_seed{args.seed}_{args.dataset}_{os.path.basename(args.model_path)}",config=config,allow_val_change=True)
