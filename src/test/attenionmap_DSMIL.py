...
def compute_attn_map(bags_list,label_file,df_test, save_path,config):
    ...
    model = DSMILNet(ins_classifier,bag_classifier).to(device=config.device)
    ...
    for i in bar:
        ...
        with torch.no_grad():
            ...
            for iteration, batch in pbar:
                ...
                # print(f"the shape of tile_pos is {tile_pos}")
           ...
           # label = torch.tensor(label).long().to(config.device)
            ins_pred, bag_pred, A, _ = model(feats)
            # print(f" Attention map is : {A} with shape {A.shape}")
            ...
            for c in range(config.classes):
                if c == prediction:
                    print(f"The slide {os.path.basename(bags_list[i])} is predicted as {class_name[c]} with probability {prediction_p[c]}")
                    attention = A[:,c].squeeze().cpu().numpy()
                    attn_df = pd.DataFrame(attention)
                    attn_df.to_csv(os.path.join(save_path, f'{slide_name}_{round(prediction_p[c],5)}_{prediction}_{label.item()}.csv'))
            ...
            # colored_tiles = (colored_tiles / num_pos_classes)
            # print(f"colored tiles shape after divided by num_pos_classes: {colored_tiles.shape}  colored tiles: {colored_tiles}")
            ...
            for k, p in enumerate(pos):
                ...
                color_map[p[0]*scale:(p[0]+1)*scale, p[1]*scale:(p[1]+1)*scale] += colored_tiles[k]
                ...
                temp_output_map = np.array(Image.blend(tile_map_img,color_map_img, 0.68))
            ...
            output_map_img.save(os.path.join(save_path, slide_name+'.png'))

            print(f"output map is save in {os.path.join(save_path, slide_name+'.png')}")
...
def main():
    ...
    class Config:
        ...
        model_path = "/system/user/publicwork/yitaocai/Master_Thesis/model/model_2C/2C_n5_seed8_SGD_determined_12212022.pt"
    ...
    save_path = "/system/user/publicwork/yitaocai/Master_Thesis/attention_map"
